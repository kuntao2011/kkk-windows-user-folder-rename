...
A complete toolkit for safely renaming a Windows user profile folder (`C:\Users\old-name`): read-only inventory → registry/hive backup → generate a foolproof rename script → execution from another admin account → batch residue-path fixing → cleanup and rollback. Typical use cases: the profile folder got truncated by Microsoft account sign-in (first 5 letters of the email), a typo in the name, or you simply want a shorter path.

## Why not just rename it

Renaming `C:\Users\<your account>` directly breaks in three places; this toolkit handles each one:
